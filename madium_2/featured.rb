...
def featured(num)
  return puts "There is no possible number that fulfills those requirements." if num >= 9_876_543_210
  first = num + 1
  odds_seven_multiples = (first..Float::INFINITY).lazy.select { |n| n % 7 == 0 && n.odd? }
  odds_seven_multiples.select do |n|
    digits_to_s = n.to_s.split('')
    digits_to_s.size == digits_to_s.uniq.size
  end.first(1)[0]
end
